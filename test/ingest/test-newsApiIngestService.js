...
const service = GhostArticleSDKClient.ingest;
...
describe('NewsApiIngestService', function () {
  ...
  describe('getPopular', () => {
    ...
    it('should return the most popular articles for a given source', () => {
      return service.getPopular({ source: 'the-new-york-times' })
      .then(articles => {
        expect(articles).to.exist;
        expect(articles).to.be.an('array');
        articles.forEach(article => {
